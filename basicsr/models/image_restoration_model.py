...
class ImageRestorationModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if self.opt.get('freeze_encoder', False):
            logger = get_root_logger()
            logger.info('[+] Freezing intro, encoders, and downs layers of NAFNet for decoder fine-tuning...')
            net = self.net_g.module if hasattr(self.net_g, 'module') else self.net_g
            if hasattr(net, 'intro'):
                net.intro.requires_grad_(False)
            if hasattr(net, 'encoders'):
                net.encoders.requires_grad_(False)
            if hasattr(net, 'downs'):
                net.downs.requires_grad_(False)
